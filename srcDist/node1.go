...
import (
	...
	"github.com/Menares-star/Tarea2/srcDist/Mensajes/Uploads"
	"github.com/Menares-star/Tarea2/srcDist/Mensajes/Propuesta"
...
func uploadsServer(lis net.Listener) {
	...
	fmt.Println("NODO1 Escuchando a nuevos clientes")
...
func proposesServer(lis net.Listener) {
	...
	fmt.Println("NODO1 Escuchando a las propuestas")
	if err := grpcServer.Serve(lis); err != nil {
		log.Fatalf("failed to serve: %s", err)
	}
}
...
func main() {
	...
	lis1, err1 := net.Listen("tcp", ":8001")
	if err1 != nil {
		log.Fatalf("failed to listen: %v", err1)
	}

	lis2, err2 := net.Listen("tcp", ":8002")
	if err2 != nil {
